...
func trackCoding(_ provider: String, arguments: [String]) throws {
    ...
    let queueURL = root.appendingPathComponent("coding-\(UUID().uuidString).json")
    ...
    guard ready.wait(timeout:.now()+5) == .success, !failed, let port = listener.port else { listener.cancel(); throw NSError(domain:"TypeGrid",code:1,userInfo:[NSLocalizedDescriptionKey:"Could not start the local metrics receiver."]) }
    ...
    queue.sync {} // Finish any accepted metrics before returning.
}
